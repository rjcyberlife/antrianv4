function startTime() {
  ...
  switch (today.getMonth()) {
    ...
    if(document.getElementById("txttanggal2") !== null)
    {
       document.getElementById('txttanggal2').innerHTML = 
    "<br><span  style='font-size:10px'><b>" + day + " , " + tgl + " " + bln + " " + thn + " " + h + ":" + m + ":" + s + "</b></span>"   ;
...
function getTotalAntrian()
{
    var xmlHttp;
        ...
        xmlHttp.onreadystatechange = function() {
            ...
            //bagian cek antrian live
            id = $('#IDLive').text();
            prefix = $('#prefixLive').text();
            no = $('#noLive').text();
            
            if (prefix=='A') {
              sisaLive = no - res.lastA;
            } else if (prefix=='B') {
              sisaLive = no - res.lastB;
            } else if (prefix=='C') {
              sisaLive = no - res.lastC;
            } else if (prefix=='F') {
              sisaLive = no - res.lastF;
            }

            if(res.id == id) {

              if(sisaLive<=0) {
                noloketpanggil(res.tanggal,prefix,no)
                
               // $("#txtsisalive").html(sisaLive);
                $("#txtsisaantrianloket").html("<h5>ANDA DILAYANI PADA LOKET </h5>");

              } else {
                $("#txtsisalive").html(sisaLive);
                $("#txtsisaantrianloket").html("<h5>SISA ANTRIAN</h5>");
...
function noloketpanggil(tanggal,prefix,no)
{
    $.ajax({
              type: "GET",
              url: "lib/ceklastpanggil.php?cat=2",
    ...
    var xmlHttp;
        ...
        xmlHttp.onreadystatechange = function() {
            ...
            //kirim data ke server
              ...
              $.post("https://antrian.kanimpekanbaru.id/lib/serviceapi.php", {data:xmlHttp.responseText,tbl:"tiket"});
...
function lastpanggil()

{
    $.ajax({
              type: "GET",
              url: "lib/ceklastpanggil.php?cat=1",
             // dataType : 'json',        
              //data: {data : xmlHttp.responseText },
              contentType : 'application/json',
             
              success: function(dt){
                $.post("https://antrian.kanimpekanbaru.id/lib/serviceapi.php", {data:dt,tbl:"panggil"});
              },
              error: function(errMsg) {
                  //your error function
              }
